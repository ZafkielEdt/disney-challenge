...
public class JwtUtils {
  ...
  private final Long EXPIRE_IN = 800000L;
  ...
  public String getUsernameFromToken(String token) throws Exception {
    ...
    try {
      ...
      throw new Exception("The username field is missing or did not have a valid value");
    }
    return username;
  }
  ...
  public String generateToken(String username, Collection<SimpleGrantedAuthority> authorities) {
    ...
    String roles = authorities.stream().map(SimpleGrantedAuthority::getAuthority)
    ...
    return Jwts.builder().setClaims(claims).setIssuedAt(new Date()).setExpiration(expiration)
        .signWith(Keys.hmacShaKeyFor(SIGNING_KEY.getBytes()), SignatureAlgorithm.HS256).compact();
  }
}
